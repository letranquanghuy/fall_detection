...
for type_ in type_path:
    print("=================================================")
    print("TYPE:", type_)
    for path in glob.glob(f"D:/HCMUT/Ths/Thesis/Evaluate/normal/{type_}/sit_to_chair/*/"):
        order_num += 1
        print(order_num)
        print(path)
        # load data
        not_fall_dataset = pd.read_csv(path+f"NOT_FALL.csv")
        # lie_dataset = pd.read_csv(path+f"LIE.csv")
        no_of_timesteps = 8
        datas = []
        labels = []
        not_fall_list = []
