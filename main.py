...
# VIDEO SOURCE
video_path = 0
video_path = 'D:/HCMUT/Ths/Thesis/deep_sort/data/test(22).mp4'
# video_path = 'D:/HCMUT/Ths/Thesis/deep_sort/data/test_persons_1.mp4'
...
weight_path = 'D:/HCMUT/Ths/Thesis/deep_sort/best24_12.engine'
...
with tf.io.gfile.GFile("D:/HCMUT/Ths/Thesis/LSTM/output/frozen_graph_23_12_2023_17_12.pb", "rb") as f:
...
while ret:
    ...
    for r in results.boxes.data.tolist():
        ...
        # frame = cv2.putText(frame, str(track_id), (x1, y1+50), cv2.FONT_HERSHEY_SIMPLEX, 2, (colors[track_id % len(colors)]), 2)
    ...
    #         # status_people[track_id]['status'] = pose_status
    #         status_people[track_id]['status'] = ''

    #     have_fall = False
    #     if track_id in track_id_list:
    #         if (not have_fall) and status_people[track_id]['status'] == "FALL":
    #             have_fall = True
    ...
    #     # if have_fall:
    #         # frame = cv2.putText(frame, 'FALL', (25, 80), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 2)
    ...
    i+=1
    if i>5:
        print('frame i', (end-start))
    if i > 20:
        break
    # frame_show = cv2.resize(frame, (int(x/2), int(y/2)))
